...
\begin{abstract}
HeLMAS began as a heterogeneous KV-bridge project: the goal was to move reasoning from a larger teacher model into a smaller student by projecting internal state across model families. That original direction is now preserved as archived history in this repository, but the current project has shifted to a cleaner and more falsifiable question: can a low-activation nested regime inside the same Gemma 3n E4B checkpoint be uplifted into the full-activation regime well enough to recover continuation behavior after handoff? We study paired low/full forward passes on the same prompt prefixes, extract matched residual-stream states and caches, and train lightweight translators that attempt to approximate the full regime from the low regime. Across completed pilot, layer-sweep, suffix-span, and targeted-site experiments, the low/full regimes are clearly separated, identity control is exact, and the best learned intervention site is a specific full-regime patch surface at 
...
The original He-LMAS idea asked whether one LLM could be made to continue another model's reasoning by transferring a cache-like internal representation. That was ambitious, but it also mixed several hard problems at once: cross-family geometry, RoPE compatibility, cache layout mismatch, and continuation stability.
...
\section{From HeLMAS to HeLMAS-3n}
The repository now contains two distinct attempts.

\subsection{Legacy attempt: heterogeneous KV transfer}
The legacy He-LMAS project attempted a cross-model bridge. The central idea was to build a RoPE-aware KV projection and a training objective that would preserve reasoning when moving from a larger teacher to a smaller student \cite{helmasLegacy2026}. The original README, now archived, framed that as a lossless reasoning transfer problem across heterogeneous Qwen checkpoints.

That attempt mattered because it motivated the broader design space, but it was not the right abstraction for the current evidence. A cross-family bridge asks too much of a single translator. It entangles geometry, model identity, and behavior in one step.

\subsection{Current attempt: same-parent regime uplift}
HeLMAS-3n makes a narrower claim. The source and target are the same Gemma 3n E4B checkpoint under two activation regimes \cite{gemma3n2025}. The learning problem is to predict the full-regime internal state from the low-regime internal state, ideally with only a small correction.

This is the key change in scientific framing:
\begin{itemize}[leftmargin=1.4em]
    \item Legacy HeLMAS: cross-model transfer.
    \item HeLMAS-3n: same-parent regime uplift.
\end{itemize}

That change is what made the experiment falsifiable.
...
\begin{table}[h]
    ...
    \input{generated/core_results_table.tex}
    \label{tab:core}
\end{table}
